...
TEST_F(ConnectionTest, TestFluentInterface)
{
  ...
  returned_conn = &conn->SetTimeout(10);
  EXPECT_EQ(conn, returned_conn) << "SetTimeout not fluent" << std::endl;
  returned_conn = &conn->SetNoSignal(true);
  EXPECT_EQ(conn, returned_conn) << "SetNoSignal not fluent" << std::endl;
  returned_conn = &conn->FollowRedirects(true);
  EXPECT_EQ(conn, returned_conn) << "FollowRedirects not fluent" << std::endl;
  returned_conn = &conn->SetUserAgent("foobar/1.2.3");
  EXPECT_EQ(conn, returned_conn) << "SetUserAgent not fluent" << std::endl;
  returned_conn = &conn->SetCAInfoFilePath("non-existent file");
  EXPECT_EQ(conn, returned_conn) << "SetCAInfoFilePath not fluent" << std::endl;
  returned_conn = &conn->SetCertPath("non-existent file");
  EXPECT_EQ(conn, returned_conn) << "SetCertPath not fluent" << std::endl;
  returned_conn = &conn->SetCertType("invalid cert type");
  EXPECT_EQ(conn, returned_conn) << "SetCertType not fluent" << std::endl;
  returned_conn = &conn->SetKeyPath("non-existent key path");
  EXPECT_EQ(conn, returned_conn) << "SetKeyPath not fluent" << std::endl;
  returned_conn = &conn->SetKeyPassword("imaginary_password");
  EXPECT_EQ(conn, returned_conn) << "SetKeyPassword not fluent" << std::endl;
  returned_conn = &conn->SetProxy("127.0.0.1:3128");
  EXPECT_EQ(conn, returned_conn) << "SetProxy not fluent" << std::endl;
  RestClient::HeaderFields headers;
  headers["Foo"] = "bar";
  headers["Bla"] = "lol";
  returned_conn = &conn->SetHeaders(headers);
  EXPECT_EQ(conn, returned_conn) << "SetHeaders not fluent" << std::endl;
  returned_conn = &conn->AppendHeader("Fizz", "buzz");
  EXPECT_EQ(conn, returned_conn) << "AppendHeader not fluent" << std::endl;
}
